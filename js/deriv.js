 ...
 * Handles: WebSocket connection, Deriv OAuth 2.0 + PKCE login, live ticks,
...
const ManooFX = (() => {
  ...
  function getRedirectUri() {
    ...
    return origin && origin !== 'null'
      ...
      : 'https://dennisfx-82td.vercel.app/dashboard.html';
  }
  ...
  async function createPkcePair() {
    ...
    const verifier = Array.from(verifierBytes, byte => alphabet[byte % alphabet.length]).join('');
  ...
  async function getOAuthUrl() {
    ...
    sessionStorage.setItem('mfx_pkce_verifier', verifier);
    sessionStorage.setItem('mfx_oauth_state', state);
  ...
  function connect() {
    ...
    ws.onopen = () => {
      ...
      startPing();
      pendingRequests.splice(0).forEach(request => ws.send(JSON.stringify(request)));
    ...
    ws.onerror = (e) => {
      ...
      errorCallbacks.forEach(callback => callback('WebSocket connection error'));
    };
  ...
  function handleAuthorize(msg) {
    if (msg.error) {
      ...
      clearToken();
      errorCallbacks.forEach(callback => callback('Login failed: ' + msg.error.message));
      return;
    }
    ...
    accountBalances[activeAccount.loginid] = activeAccount;
    ...
    loginCallbacks.forEach(callback => callback(activeAccount));
  ...
  function fetchAccountBalances() {
    ...
    tokens.forEach(account => {
      if (!account.token || account.loginid === activeAccount?.loginid) return;

      const accountWs = new WebSocket(WS_URL);
      const timeout = setTimeout(() => accountWs.close(), 10000);

      accountWs.onopen = () => accountWs.send(JSON.stringify({ authorize: account.token }));
      accountWs.onmessage = event => {
        ...
        if (message.msg_type === 'authorize') {
          ...
          accountWs.send(JSON.stringify({ balance: 1 }));
        } else if (message.msg_type === 'balance' && message.balance) {
          accountBalances[account.loginid] = {
            loginid: account.loginid,
            currency: message.balance.currency,
            balance: message.balance.balance,
            is_virtual: account.loginid?.startsWith('VRT') ? 1 : 0,
          };
          accountBalancesCallbacks.forEach(callback => callback(accountBalances));
          clearTimeout(timeout);
          accountWs.close();
        }
      };
  ...
  function handleBalance(msg) {
    ...
    accountBalances[activeAccount.loginid] = activeAccount;
    sessionStorage.setItem('mfx_account', JSON.stringify(activeAccount));
    balanceCallbacks.forEach(callback => callback(balance, msg.balance.currency));
    accountBalancesCallbacks.forEach(callback => callback(accountBalances));
  ...
  function handleProposal(msg) {
    if (msg.error) {
      errorCallbacks.forEach(callback => callback('Proposal error: ' + msg.error.message));
      return;
    }
  ...
  function handleBuy(msg) {
    if (msg.error) {
      errorCallbacks.forEach(callback => callback('Trade error: ' + msg.error.message));
  ...
  function handleError(msg) {
    ...
    errorCallbacks.forEach(callback => callback(errMsg));
  }

  // ─── OAUTH ─────────────────────────────────────────────────
  async function loginWithDeriv() {
    sessionStorage.setItem('mfx_return', getRedirectUri());
  ...
  async function handleOAuthCallback() {
    ...
    if (params.get('error')) {
      ...
      sessionStorage.setItem('mfx_oauth_error', message);
    ...
    const savedState = sessionStorage.getItem('mfx_oauth_state');
    const verifier = sessionStorage.getItem('mfx_pkce_verifier');
    if (!code || !returnedState || returnedState !== savedState || !verifier) {
      sessionStorage.setItem('mfx_oauth_error', 'Invalid OAuth callback or state.');
      return null;
    }
    ...
    sessionStorage.removeItem('mfx_oauth_state');
    sessionStorage.removeItem('mfx_pkce_verifier');
    window.history.replaceState({}, document.title, window.location.pathname);
    if (!response.ok || !data.access_token) {
      sessionStorage.setItem('mfx_oauth_error', data.error_description || data.error || 'OAuth token exchange failed.');
      return null;
    }
  ...
  // ─── EVENT CALLBACKS ───────────────────────────────────────
  function onLogin(fn)   { loginCallbacks.push(fn); }
  function onBalance(fn) { balanceCallbacks.push(fn); }
  function onError(fn)   { errorCallbacks.push(fn); }
  function onAccountBalances(fn) { accountBalancesCallbacks.push(fn); }
  ...
  async function init() {
    ...
    const callbackData = `${window.location.search}&${window.location.hash}`;
    if (callbackData.includes('code=') || callbackData.includes('error=')) {
      const t = await handleOAuthCallback();
      if (t) {
        connect();
        return;
      }
