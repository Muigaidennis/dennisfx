...
function initDashboard() {
  ...
  const realTab = document.querySelector('[data-mfx-account="real"]');
  const demoTab = document.querySelector('[data-mfx-account="demo"]');
  const accountTabs = [realTab, demoTab].filter(Boolean);

  function renderAccounts(accounts) {
    const values = Object.values(accounts);
    const real = values.find(account => !Number(account.is_virtual));
    const demo = values.find(account => Number(account.is_virtual));
    const active = ManooFX.getAccount();

    const realBalance = document.getElementById('realBalance');
    const demoBalance = document.getElementById('demoBalance');
    if (realBalance) realBalance.textContent = real ? `${real.currency} ${parseFloat(real.balance).toFixed(2)}` : 'Unavailable';
    if (demoBalance) demoBalance.textContent = demo ? `${demo.currency} ${parseFloat(demo.balance).toFixed(2)}` : 'Unavailable';

    if (active) {
      const activeBalance = document.getElementById('dashBalance');
      if (activeBalance) activeBalance.textContent = `${active.currency} ${parseFloat(active.balance).toFixed(2)}`;
      document.querySelectorAll('[data-mfx-role]').forEach(el => {
        el.textContent = `${Number(active.is_virtual) ? 'Demo' : 'Real'} · ${active.loginid || ''}`;
      });
    }
  }

  accountTabs.forEach(tab => tab.addEventListener('click', () => {
    const account = Object.values(ManooFX.getAccountBalances()).find(item =>
      tab.dataset.mfxAccount === 'demo' ? Number(item.is_virtual) : !Number(item.is_virtual)
    );
    if (!account) {
      showToast(`${tab.textContent.trim()} is not available for this Deriv login.`, 'error');
      return;
    }
    ManooFX.switchAccount(account.loginid);
  ...
  ManooFX.onAccountBalances(renderAccounts);
  renderAccounts(ManooFX.getAccountBalances());
